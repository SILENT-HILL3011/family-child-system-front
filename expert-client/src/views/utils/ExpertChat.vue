...
const expertId = ref(localStorage.getItem('expertId') || '')
...
const connectNetty = () => {
  ...
  ws.onopen = () => {
    console.log('专家 Netty 已连接')
    // 绑定专家ID
    ws.send('BIND_USER:' + expertId.value)
  }
...
const sendMessage = async () => {
  ...
  if (!boardId.value) return ElMessage.warning('无效会话')

  try {
    // 1. 调用回复接口
    const res = await applyMessage({ content, boardId: boardId.value })
    if (res.code === 200) {
      inputText.value = ''
      await loadHistory()
